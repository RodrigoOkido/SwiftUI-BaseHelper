...
struct MapView: View {
    ...
    // Region: This coordinates is LONDON
    ...
    var body: some View {
        ZStack {
            Map(coordinateRegion: $region, showsUserLocation: viewModel.region != nil ? true : false, userTrackingMode: .constant(.follow))
                .edgesIgnoringSafeArea([.top, .leading, .trailing])
                .onAppear {
                    viewModel.checkUserAuthorizationStatus()
                    #warning("If you want to test your real location...uncomment this code. Works only on real device")
